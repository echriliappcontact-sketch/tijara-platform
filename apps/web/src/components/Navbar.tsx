  ...
  return (
    <nav style={{ background: "#111", borderBottom: "1px solid #222", padding: "0 24px", direction: "ltr" }}>
      <div style={{ maxWidth: 1200, margin: "0 auto", display: "flex", alignItems: "center", justifyContent: "space-between", height: 60 }}>
        <Link href="/" style={{ fontSize: 22, fontWeight: 700, color: "#10b981", textDecoration: "none" }}>Tijara</Link>
        <div style={{ display: "flex", gap: 16, alignItems: "center" }}>
          ...
          {logged ? (
            <>
              ...
              <Link href="/dashboard/settings" style={{ color: "#888", textDecoration: "none", fontSize: 13 }}>Settings</Link>
              <span style={{ color: "#555", fontSize: 12 }}>{user?.email}</span>
              <button onClick={logout} style={{ background: "#dc2626", color: "white", padding: "5px 12px", borderRadius: 6, border: "none", cursor: "pointer", fontSize: 12 }}>{t("nav.logout")}</button>
            </>
          ) : (
            <>
              <Link href="/login" style={{ color: "#ccc", textDecoration: "none", fontSize: 14 }}>{t("nav.login")}</Link>
              <Link href="/register" style={{ background: "#10b981", color: "white", padding: "7px 14px", borderRadius: 6, textDecoration: "none", fontSize: 13, fontWeight: 600 }}>{t("nav.register")}</Link>
            </>
          )}
          <div style={{ position: "relative" }}>
            ...
            {showLang && (
              <div style={{ position: "absolute", top: "100%", right: 0, marginTop: 4, background: "#1a1a1a", border: "1px solid #333", borderRadius: 83", borderRadius: 6, padding: "5px 10px", color: "#ccc", cursor, overflow: "hidden", zIndex: 100, minWidth: 120 }}>
                {languages.map((l) => (
