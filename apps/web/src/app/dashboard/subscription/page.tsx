...
export default function SubscriptionPage() {
  ...
  const loadData = async () => {
    try {
      const [plansRes, subRes, payRes] = await Promise.all([
        ...
        api.get("/subscriptions/my-payments/" + store?.id),
      ]);
  ...
  const handleReceiptUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      reader.onload = async () => {
        ...
        } catch (e: any) { setErr("Failed to upload receipt"); }
  ...
  const submitPayment = async (e: React.FormEvent) => {
    ...
    if (!transactionRef) { setErr("Please enter transaction number"); setSubmitting(false); return; }
    if (!receiptUrl) { setErr("Please upload payment receipt"); setSubmitting(false); return; }
    try {
      ...
      loadData();
    } catch (e: any) { setErr(e.response?.data?.message || "Failed to submit payment"); }
  ...
  return (
    <div style={{ padding: "30px 24px", maxWidth: 900, margin: "0 auto" }}>
      ...
      {subscription && step === "current" && (
        <div className="card" style={{ marginBottom: 24, background: isActive ? "#0d2818" : isTrial ? "#1a1a0a" : "#2d1215", border: "1px solid " + (isActive ? "#10b981" : isTrial ? "#eab308" : "#dc2626") }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
            ...
            <div>
              <div style={{ fontSize: 13, color: "#888" }}>Start Date</div>
              <div style={{ fontSize: 14 }}>{subscription.startDate ? new Date(subscription.startDate).toLocaleDateString() : "-"}</div>
            </div>
            <div>
              <div style={{ fontSize: 13, color: "#888" }}>End Date</div>
              <div style={{ fontSize: 14 }}>{subscription.endDate ? new Date(subscription.endDate).toLocaleDateString() : "-"}</div>
            </div>
            <div>
            ...
            {subscription.plan && (
              <div>
                <div style={{ fontSize: 13, color: "#888" }}>Limits</div>
                <div style={{ fontSize: 13 }}>{subscription.plan.maxProducts || "Unlimited"} Products / {subscription.plan.maxOrders || "Unlimited"} Orders</div>
              </div>
            )}
          </div>
          {!isActive && (
            <button onClick={() => { setStep("plans"); setSelectedPlan(null); }} style={{ marginTop: 16, background: "#10b981", color: "white", padding: "10px 24px", borderRadius: 8, border: "none", cursor: "pointer", fontWeight: 600 }}>
      ...
      {step === "plans" && (
        <div>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
            <h2 style={{ fontSize: 18, fontWeight: 600 }}>Choose a Plan</h2>
            <button onClick={() => setStep("current")} style={{ padding: "6px 14px", borderRadius: 6, border: "1px solid #333", background: "transparent", color: "#aaa", cursor: "pointer" }}>Back</button>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 16, marginBottom: 24 }}>
            {plans.map((plan: any) => (
              <div key={plan.id} onClick={() => { setSelectedPlan(plan); setStep("payment"); }} style={{ background: "#111", borderRadius: 12, border: plan.isPopular ? "2px solid #10b981" : "1px solid #333", padding: 24, cursor: "pointer", textAlign: "center" }}>
                {plan.isPopular && <div style={{ background: "#10b981", color: "white", padding: "4px 12px", borderRadius: 20, fontSize: 11, fontWeight: 600, display: "inline-block", marginBottom: 8 }}>Most Popular</div>}
                <h3 style={{ fontSize: 20, fontWeight: 700, marginBottom: 8 }}>{plan.name}</h3>
                <div style={{ fontSize: 32, fontWeight: 800, color: "#10b981", marginBottom: 12 }}>{plan.price?.toLocaleString()} <span style={{ fontSize: 14, fontWeight: 400 }}>DZD / {plan.duration} days</span></div>
                ...
                <div style={{ marginTop: 16, background: "#1a1a1a", padding: "8px 16px", borderRadius: 6, color: "#10b981", fontSize: 13, fontWeight: 600 }}>Select Plan</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="card" style={{ border: "1px solid #10b981" }}>
          ...
          <div style={{ background: "#1a1a1a", borderRadius: 8, padding: 16, marginBottom: 20 }}>
            ...
            <div style={{ fontSize: 13, lineHeight: 2, color: "#aaa" }}>
              ...
              <div>Account Name: <strong>Tijara Platform</strong></div>
              <div>Amount: <strong style={{ color: "#10b981", fontSize: 16 }}>{selectedPlan.price?.toLocaleString()} DZD</strong></div>
            </div>
          </div>
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              {receiptUrl && (
                <div style={{ marginTop: 12 }}>
                  <img src={receiptUrl} alt="Payment receipt" style={{ maxWidth: 200, maxHeight: 200, borderRadius: 8, border: "1px solid #333" }} />
                  <div style={{ color: "#10b981", fontSize: 12, marginTop: 4 }}>Receipt uploaded!</div>
                </div>
              )}
            </div>
            <div style={{ display: "flex", gap: 8 }}>
              <button type="submit" className="btn-primary" disabled={submitting || uploading}>{submitting ? "Submitting..." : "Submit Payment"}</button>
              <button type="button" onClick={() => { setStep("plans"); setReceiptUrl(""); setTransactionRef(""); }} style={{ padding: "10px 20px", borderRadius: 8, border: "1px solid #333", background: "transparent", color: "#aaa", cursor: "pointer" }}>Back</button>
            </div>
          </form>
        </div>
      )}
      ...
      {/* Payment History */}
      {payments.length > 0 && step === "current" && (
        <div className="card">
